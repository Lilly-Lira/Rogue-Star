...
const clampFactor = (value: number) => Math.max(0, Math.min(3, value));
...
const resolveReferencePartId = (layerKey: string): string | null => {
...
const applyBodyColorToPreview = (
  ...
  const next = preview.map((entry) => {
    ...
    const layers = (entry.layers || []).map((layer) => {
      ...
      const recolored = recolorGrid(layer.grid, base, target);
